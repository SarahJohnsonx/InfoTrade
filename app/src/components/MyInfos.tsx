import { useAccount } from 'wagmi';
import { useInfoTradeRead, useInfoBasicDetails, useInfoTradeWrite } from '../hooks/useInfoTrade';
import { useState, useEffect } from 'react';
import { InfoCard } from './InfoCard';

export function MyInfos() {
  const { address } = useAccount();
  const { userInfoItems, refetch } = useInfoTradeRead();
  const { grantAccess, deactivateInfo, loading, error } = useInfoTradeWrite();
  const [refreshKey, setRefreshKey] = useState(0);

  const forceRefresh = () => {
    setRefreshKey(prev => prev + 1);
    refetch();
  };

  useEffect(() => {
    const interval = setInterval(() => {
      refetch();
    }, 10000);

    return () => clearInterval(interval);
  }, [refetch]);

  const handleGrantAccess = async (infoId: number, buyer: string) => {
    try {
      await grantAccess(infoId, buyer);
      forceRefresh();
      alert('访问权限已授予！');
    } catch (err) {
      console.error('Grant access failed:', err);
      alert('授权失败：' + (err as Error).message);
    }
  };

  const handleDeactivate = async (infoId: number) => {
    if (!confirm('确定要停用这条信息吗？停用后将无法被新用户购买。')) {
      return;
    }

    try {
      await deactivateInfo(infoId);
      forceRefresh();
      alert('信息已停用！');
    } catch (err) {
      console.error('Deactivate failed:', err);
      alert('停用失败：' + (err as Error).message);
    }
  };

  if (!userInfoItems || userInfoItems.length === 0) {
    return (
      <div style={{
        textAlign: 'center',
        padding: '4rem 0',
        color: '#6b7280'
      }}>
        <h2 style={{ fontSize: '1.5rem', marginBottom: '1rem' }}>您还没有创建任何信息</h2>
        <p>去创建您的第一条信息，开始在InfoTrade上分享有价值的内容吧！</p>
      </div>
    );
  }

  return (
    <div>
      <div style={{
        display: 'flex',
        justifyContent: 'space-between',
        alignItems: 'center',
        marginBottom: '2rem'
      }}>
        <h1 style={{ fontSize: '2rem', fontWeight: 'bold', color: '#1f2937' }}>
          我的信息 ({userInfoItems.length})
        </h1>
        <button
          onClick={forceRefresh}
          disabled={loading}
          style={{
            padding: '0.5rem 1rem',
            backgroundColor: '#3b82f6',
            color: 'white',
            border: 'none',
            borderRadius: '0.375rem',
            cursor: loading ? 'not-allowed' : 'pointer',
            opacity: loading ? 0.5 : 1,
          }}
        >
          {loading ? '刷新中...' : '刷新'}
        </button>
      </div>

      {error && (
        <div style={{
          backgroundColor: '#fee2e2',
          border: '1px solid #fecaca',
          borderRadius: '0.375rem',
          padding: '1rem',
          marginBottom: '1rem',
          color: '#dc2626'
        }}>
          错误：{error}
        </div>
      )}

      <div style={{
        display: 'grid',
        gap: '1.5rem',
        gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))'
      }}>
        {userInfoItems.map(infoId => (
          <MyInfoItem
            key={`${Number(infoId)}-${refreshKey}`}
            infoId={Number(infoId)}
            onGrantAccess={handleGrantAccess}
            onDeactivate={handleDeactivate}
            userAddress={address}
          />
        ))}
      </div>
    </div>
  );
}

interface MyInfoItemProps {
  infoId: number;
  onGrantAccess: (infoId: number, buyer: string) => void;
  onDeactivate: (infoId: number) => void;
  userAddress?: string;
}

function MyInfoItem({ infoId, onGrantAccess, onDeactivate, userAddress }: MyInfoItemProps) {
  const { data: basicDetails, refetch } = useInfoBasicDetails(infoId);

  useEffect(() => {
    const interval = setInterval(() => {
      refetch();
    }, 15000);

    return () => clearInterval(interval);
  }, [refetch]);

  if (!basicDetails) {
    return (
      <div style={{
        backgroundColor: 'white',
        borderRadius: '0.5rem',
        boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1)',
        padding: '1.5rem',
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center',
        minHeight: '200px'
      }}>
        <div style={{ color: '#9ca3af' }}>加载中...</div>
      </div>
    );
  }

  const isOwner = userAddress?.toLowerCase() === basicDetails.owner.toLowerCase();
  const canGrantAccess = isOwner && basicDetails.hasPurchased && !basicDetails.hasAccess;
  const canDeactivate = isOwner && basicDetails.isActive;

  return (
    <div style={{
      backgroundColor: 'white',
      borderRadius: '0.5rem',
      boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1)',
      padding: '1.5rem',
      border: !basicDetails.isActive ? '2px solid #fbbf24' : '1px solid #e5e7eb'
    }}>
      <InfoCard
        infoId={infoId}
        basicDetails={basicDetails}
        isOwner={isOwner}
        canPurchase={false}
        canGrantAccess={canGrantAccess}
        onPurchase={() => {}}
        onGrantAccess={(buyer) => onGrantAccess(infoId, buyer)}
      />

      {/* Additional owner actions */}
      <div style={{
        marginTop: '1rem',
        paddingTop: '1rem',
        borderTop: '1px solid #e5e7eb',
        display: 'flex',
        gap: '0.75rem'
      }}>
        {canDeactivate && (
          <button
            onClick={() => onDeactivate(infoId)}
            style={{
              backgroundColor: '#ef4444',
              color: 'white',
              padding: '0.5rem 1rem',
              borderRadius: '0.375rem',
              border: 'none',
              fontSize: '0.875rem',
              fontWeight: '500',
              cursor: 'pointer'
            }}
          >
            停用信息
          </button>
        )}

        <div style={{
          fontSize: '0.75rem',
          color: '#6b7280',
          display: 'flex',
          alignItems: 'center',
          marginLeft: 'auto'
        }}>
          {basicDetails.hasPurchased && !basicDetails.hasAccess && (
            <span style={{ color: '#f59e0b' }}>⚠️ 有待授权的购买</span>
          )}
          {!basicDetails.isActive && (
            <span style={{ color: '#6b7280' }}>ℹ️ 已停用</span>
          )}
        </div>
      </div>
    </div>
  );
}